...
class GINConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        if edge_attr is not None:
            ...
            out = self.mlp((1 + self.eps).to("cuda:0") * x +
...
class GCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # edge_weight = torch.ones((edge_index.size(1), ), =edge_index.device)
...
class GNN_node(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, drop_ratio=0.0, JK="last", residual=False, gnn_type='gin', feat_dim=None, perturb_position='X'):
        ...
        self.node_encoder = torch.nn.Embedding(
            feat_dim, emb_dim)  # uniform input node embedding
    ...
    def reset_parameters(self):
        ...
        for module in self.batch_norms:
            module.reset_parameters()
